...
my_server <- function(input, output) {
  ...
  output$linechart_em <- renderPlot({
    ...
    ggplot(first_chart_data, aes(x = as.numeric(year))) +
      ...
      annotate(
        geom = "text", x = max(input$year), y = 300,
        label = "  CO  ", hjust = "left"
      ) +
      geom_line(aes(y = O3_levels), color = "orchid4", size = 1.5) +
      annotate(
        geom = "text", x = max(input$year), y = 28,
        label = "  O3  ", hjust = "left"
      ) +
      geom_line(aes(y = NO2_levels), color = "forestgreen", size = 1.5) +
      annotate(
        geom = "text", x = max(input$year), y = 12,
        label = "  NO2  ", hjust = "left"
      ) +
      geom_line(aes(y = SO2_levels), color = "turquoise4", size = 1.5) +
      annotate(
        geom = "text", x = max(input$year), y = -3,
        label = "  SO2  ", hjust = "left"
      ) +
